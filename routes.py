...
def login():
    ...
    if request.method == 'POST':
        ...
        if user:
            login_attempts = get_user_login_attempts(user)
            if check_password_hash(user.password, password):
                ...
                user.last_attempt_time = datetime.now()
                db.session.commit()
                if 'error_count' not in session or session.get('user_id') != user.id:
                    session['error_count'] = 1
                    session['user_id'] = user.id
                else:
                    session['error_count'] += 1
        else:
            if 'error_count' not in session or session.get('user') != username:
                session['error_count'] = 1
        ...
        if error_count >= 5:
            ...
            flash(f'The ID or password you entered is incorrect. Attempts remaining: {5 - error_count}', 'error')
        return redirect(url_for('routes.login'))
    
    error_count = session.get('error_count', 0)
    return render_template('auth/login.html', error_count=error_count)
